...
const Login: React.FC = () => {
  ...
  // console.log({ cookie: cookie.ga_token });
  ...
  const signIn = async (e: any) => {
    e.preventDefault();
    setLoading(true);
    const response = await fetch(`${userservice}/oauth2/token/v2`, {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      setCookie("ga_token", res.access_token);
