...
const ActivityInvite = ({ open, onClose, fetchData, activityRole }: FilterPropsType) => {
  ...
  const handleOk = async () => {
    const response = await fetch(
      `${globalalohaservice}/v1/activity/${id}/members`,
      {
        method: "POST",
        headers: {
              ...
              {result.map((item: any) => (
                <p
                  ...
                  onClick={() => {
                    ...
                    newItem.push({
                      Body: `Hi,<br>I would like you to join my activity: <strong>${item.firstName}</strong>`,
                      Email: item.email,
                      Name: item.firstName || "",
