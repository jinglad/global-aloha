...
const Library: React.FC<LibraryProps> = ({ categories }) => {
  ...
  useEffect(() => {
    const newCategories = categories[0].Items?.filter((category:any) => category.ActivityTypes !== null);
  ...
  useEffect(() => {
    getLibraryList(categoryTitle, 12, page, dispatch);
  }, [categoryTitle]);
          ...
          <div className="py-5">
            ...
            <div className="ml-16">
              <p
                className={
                  categoryTitle === ""
                    ? "cursor-pointer bg-blue-500 w-32 py-2 px-3 rounded mt-0 mb-1 text-white"
